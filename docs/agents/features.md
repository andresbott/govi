...
| Action registry (17 actions: play-pause, stop, seek ±, volume ±, mute, progress, info, help, fullscreen, next/previous-video, move-to-trash, delete-file, preferences, quit) | `app/player/actions.go` |
...
| Key auto-repeat for held keys, throttled per action (`action.repeat`): seek/volume ~50 ms, next/previous 300 ms since each fire loads a file; press-only actions (quit, delete, play-pause…) ignore repeat | `app/player/actions.go` (`repeat` field), `app/player/input.go` (`dispatchKey`) |
